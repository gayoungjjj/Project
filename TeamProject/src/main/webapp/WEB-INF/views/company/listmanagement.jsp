    ...
    <tr>
      <td colspan="4">
       <a class="btn btn-outline-secondary" 
            href="/Company/WriteForm2?aplnum=${vo.aplnum}&user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">새로운 공고 쓰기</a></td>
     </tr>
     <tr>
      <td>공고 번호</td>
      <td>공고명</td>
     ...
     <c:forEach var="main" items="${CompanyList}">
      <tr>
       <td>${ main.aplnum   }</td>
       <td>
       <a href="/Company/Postview2?aplnum=${main.aplnum}&user_id=${sessionScope.login.user_id}&compname=${sessionScope.login.compname}">
       ${ main.post_id  }</a>
       </td>
       <td>${ main.hit }</td>
       <td>${ main.deadline }</td>
